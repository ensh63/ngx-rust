...
impl Request {
    ...
    pub fn discard_request_body(&mut self) -> NgxResult {
        unsafe { ngx_make_result(ngx_http_discard_request_body(&mut self.0)) }
    }
    ...
    pub fn send_header(&mut self) -> NgxResult {
        unsafe { ngx_make_result(ngx_http_send_header(&mut self.0)) }
    }
    ...
    pub fn output_filter(&mut self, body: &mut ngx_chain_t) -> NgxResult {
        unsafe { ngx_make_result(ngx_http_output_filter(&mut self.0, body)) }
    }
